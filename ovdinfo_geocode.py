...
for prisoner in ovd['data']:
    ...
    for address in filter(bool, addresses):
        addresssafe = address.translate({ord('«') : ' ', ord('»') : ' ', ord('"') : ' ', ord("№") : ' ', ord('|') : ','}).replace('  ', ' ')
        if address not in cache or args.cache_override:
            places = json.load(urllib.request.urlopen(urllib.request.Request(args.google_places_api_url, data = json.dumps(dict(textQuery = addresssafe)).encode('utf-8'), method = 'POST', headers = {'Content-Type': 'application/json', 'X-Goog-Api-Key' : secrets.get('GOOGLE_API_KEY', ''), 'X-Goog-FieldMask': 'places.id,places.displayName,places.formattedAddress,places.googleMapsUri,places.location'})))
            print('Places:', len(places['places']))
            for placeres in places['places']:
                placeid = placeres['id']
