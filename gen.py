...
parser.add_argument('--ovd-info-dataset-url', default = 'https://api.repression.info/v1/data', help = 'curl https://api.repression.info/v1/data | jq > ovdinfo.json')
...
data = [dict(prisoner, **addresses[sanitized_addresses[0]]) for prisoner in ovd['data'] for sanitized_addresses in [list(filter(bool, map(sanitize_addr, prisoner["detention_center_ru"] + [prisoner["persecution_location_ru"]] + prisoner["penal_facility_ru"] + [prisoner["imprisonment_location_ru"]])))] if sanitized_addresses and sanitized_addresses[0] in addresses] 
